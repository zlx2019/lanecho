// English locale; `satisfies Locale` enforces key parity with zh.ts at compile time.

import type { Locale } from "./index";

export const en = {
  header: {
    tagline: "Shared clipboard over LAN",
    toLight: "Switch to light",
    toDark: "Switch to dark",
    toSystem: "Follow system",
  },
  nav: {
    general: "General",
    devices: "Devices",
    sync: "Sync",
    storage: "Storage",
    ignore: "Ignore",
    hotkeys: "Hotkeys",
  },
  ignore: {
    paneApps: "Apps",
    paneTypes: "Pasteboard Types",
    paneRegex: "Regex",
    paneFiles: "Files",
    appsNote: "Ignore text copied from these applications.",
    appsUnsupported: "This platform cannot identify the source application; the rule is unavailable.",
    typesNote:
      "Content carrying any of these clipboard types is ignored; the preset list covers common password managers and transient markers.",
    regexNote:
      "Patterns search the text as written; any hit ignores it. Anchor with ^ $ for an exact match.",
    filesNote:
      "One rule per line, # starts a comment; a pattern with / matches the full path, otherwise the file name; matched files are dropped, the rest go through.",
    suppressSync: "Don't sync",
    suppressRecord: "Don't record",
    reset: "Reset",
    empty: "No entries",
    inlinePlaceholder: "Type and press Enter to add, Esc to cancel",
    regexLiteral: "plain text",
    regexLiteralHint: "Not a valid pattern here, matched as a plain substring",
  },
  sync: {
    toggle: "Clipboard sync",
    toggleHint: "When off, nothing is broadcast or received",
    mode: "Sync policy",
    modeOff: "Off",
    modeBoth: "Two-way",
    modeSend: "Send only",
    modeReceive: "Receive only",
    modeHintOff: "Sync fully off - local clipboard history only",
    modeHintBoth: "Broadcast your copies and receive from other devices",
    modeHintSend: "Send your copies out, receive nothing",
    modeHintReceive: "Receive from other devices, send nothing",
    types: "Synced types",
    typeText: "Text",
    typeImages: "Images",
    typeFiles: "Files",
    typesHint: "Only checked types are sent and received",
    fileLimit: "File size limit",
    fileLimitHint: "MB; larger files stay local (1-512)",
    lastFrom: (name: string) => `Synced from ${name}`,
    idle: "What you copy syncs instantly to paired devices",
  },
  devices: {
    empty: "No other devices found — open Lanecho on another machine",
    online: "online",
    offline: "offline",
    paired: "paired",
    pair: "Pair",
    unpair: "Unpair",
    unpairConfirm: "Confirm",
    pairing: "Waiting…",
    pairSent: (name: string) => `Pairing request sent to ${name}, waiting for confirmation`,
  },
  pairing: {
    title: "pairing request",
    message: (name: string) => `${name} wants to pair for clipboard sync`,
    hint: "Once paired, what you copy on either device lands on the other's clipboard. You can unpair anytime.",
    fingerprint: "Their fingerprint",
    accept: "Accept",
    decline: "Decline",
  },
  settings: {
    nickname: "Device name",
    nicknamePlaceholder: "Defaults to hostname",
    port: "Listen port",
    portHint: "Takes effect after restart",
    language: "Language",
    theme: "Theme",
    themeSystem: "System",
    themeLight: "Light",
    themeDark: "Dark",
    previewDelay: "Preview delay",
    previewDelayHint: "ms before the detail card appears; 0 = instant",
    autostart: "Launch at login",
    notifyOnSync: "Sync notification",
    notifyOnSyncHint: "Notify when another device overwrites the clipboard",
    fingerprint: "This device's fingerprint",
    copy: "Copy",
    copied: "Copied",
    port_self: (port: number) => `port ${port}`,
  },
  history: {
    searchPlaceholder: "Search history…",
    empty: "No history yet — try copying something",
    noMatch: "No matching entries",
    imageLabel: (size: string) => `Image ${size}`,
    fromDevice: (name: string) => `from ${name}`,
    pin: "Pin",
    unpin: "Unpin",
    delete: "Delete",
    clear: "Clear",
    clearConfirm: "Clear all history?",
    clearHint: (n: number) =>
      `Deletes all ${n} entries, including pinned ones. This cannot be undone.`,
    cancel: "Cancel",
    count: (n: number) => `${n} item${n === 1 ? "" : "s"}`,
    settings: "Settings",
    about: "About",
    quit: "Quit",
  },
  preview: {
    app: "Application",
    fromDevice: "From device",
    firstCopied: "First copied",
    lastCopied: "Last copied",
    copyCount: "Copies",
    truncated: (n: number) => `Too long — showing first ${n} characters`,
  },
  historySettings: {
    maxEntries: "Keep entries",
    recordTypes: "Record types",
    recordText: "Text",
    recordImages: "Images",
    recordFiles: "Files",
    sort: "Sort order",
    sortRecent: "Most recent",
    sortFrequent: "Most copied",
    panelHotkey: "Panel hotkey",
    panelHotkeyHint: "Click, then press the new combination; Esc cancels",
    hotkeyUnset: "Not set",
    hotkeyPressKeys: "Press keys…",
    hotkeyClear: "Clear (disables the panel hotkey)",
    slotHotkeys: "Quick paste (1~6)",
    slotHotkeysHint: "Top 6 entries in the panel's current order",
    slotModifier: "Modifier",
    slotModifierHint: "Combined with digits 1~6 to restore an entry directly",
    slotConflict: (combos: string) => `${combos} taken by another app, not active`,
    autoPaste: "Paste after selecting",
    autoPasteHint: "Sends the paste key to the frontmost app once you pick an entry",
    autoPastePermission:
      "Accessibility permission required: enable Lanecho under System Settings → Privacy & Security → Accessibility, then restart Lanecho.",
    autoPasteGrant: "Grant",
    usage: (size: string) => `Using ${size}`,
    incognito: "Pause recording",
    incognitoHint: "Nothing is added to history while on (this session only)",
  },
  about: {
    tagline: "Shared clipboard over LAN",
    version: "Version",
    repo: "Project page",
    license: "Open source under the MIT licence",
    copyright: "Copyright © 2026 zlx2019",
  },
  errors: {
    peer_unreachable: "Peer unreachable — make sure it is online",
    pair_rejected: "Peer declined the pairing request",
    fingerprint_mismatch: "Peer identity check failed (fingerprint mismatch)",
    timeout: "Timed out waiting for the peer",
    not_paired: "Not paired with this device yet",
    too_large: "Content exceeds the peer's size limit",
    disabled: "Peer has paused syncing",
    unsupported_type: "Peer does not support this content type",
    identity_mismatch: "The peer could not confirm this device's identity",
    unsupported_version: "The peer does not support this protocol version",
    rejected: "Peer rejected this sync",
    settings_save_failed: "Failed to save settings",
    rename_failed: "Failed to update device name",
    engine: "Engine error",
    hotkey_invalid: "Hotkey is invalid or already taken",
    app_resolve_failed: "Could not identify the selected application",
    history_missing: "History entry is missing or corrupted",
    files_missing: "Source files were moved or deleted",
    clipboard_write_failed: "Failed to write to the clipboard",
    checksum_mismatch: "Transfer checksum mismatch; content discarded",
  },
} satisfies Locale;
