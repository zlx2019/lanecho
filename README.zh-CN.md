...
每台运行着 Lanecho 程序的设备，都会成为局域网内的一个节点。多个节点之间可以相互匹配，形成群组，组内的节点将会共享对方的剪切板。

设备之间只需配对一次，此后在其中一台上复制的东西，就能直接在其余设备上粘贴，全程在设备之间点对点传输，走双向认证的 TLS 1.3 通道。最近复制过的内容都留在一份可搜索的历史里，一个快捷键就能唤起。

## ✨ 功能特性

- 🔗 **局域网p2p** —— 设备之间数据直传。无需**服务端**/**云端**。
- ⚡ **即时同步** —— 复制数据后会立刻同步到其他设备的剪切板并存储到历史列表中，并且一次复制仅传播一次。
- 📋 **剪贴板历史** —— 历史复制记录，通过快捷键（默认 `Cmd/Ctrl+Shift+C`）可唤起历史列表面板，快速选择曾经复制过的数据。可通过 `Cmd/Ctrl+1..6` 快捷键（修饰键可在设置中更改）把前六条里的任意一条直接放回剪贴板。鼠标悬浮则展示复制记录详情。
- ⌨️ **选中后自动粘贴** —— 可选项：从历史里选中一条后，自动向你刚才所在的应用发送粘贴按键（默认关闭；macOS 需要授予辅助功能权限）。
...
- 🚫 **忽略规则** —— 用自定义规则把内容挡在外面：按来源应用、剪贴板类型标记、文本正则或文件名模式匹配，每条规则可分别选择忽略同步、忽略记录或两者皆忽略。
...
**请用原生版。** 它用 Swift 与 AppKit 写成，进程里没有 WebView —— 内存占用更小，行为也更像一个 Mac 应用：面板不会从你正在打字的地方夺走焦点。
...
- **还可以自己定规则。** 有些密码管理器根本不打任何标记 —— Apple 自家的「密码」App 就是其一 —— 上面的豁免机制对它们无能为力。此时可以为该应用添加一条忽略规则（或按剪贴板类型、文本正则、文件名模式匹配），并按规则分别选择忽略同步、忽略记录或两者皆忽略。
...
**更新版本后「选中后自动粘贴」失效了（macOS）。**
未签名构建的辅助功能授权与二进制本体绑定，每次更新都会让旧授权作废 —— 而且在列表里重新勾选旧条目是没用的。请在「系统设置 → 隐私与安全性 → 辅助功能」中移除 Lanecho，重新打开应用并授权新条目，然后完全退出并重启 Lanecho。等发布版签名后此问题会自然消失。
