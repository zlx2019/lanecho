...
- ↔️ **逐设备的同步方向** —— 每台已配对设备都可单独设为双向、只发送、只接收或关闭，公用或不太信任的机器可以只收不发。
...
- **每台设备能做什么由你定。** 记录与同步是两组独立开关，可按内容类型分别控制，且每台已配对设备都有自己的方向设置。
...
**Linux 上有哪些限制？**
剪贴板访问经由 X11（Wayland 合成器下走 XWayland）。X11 上双向同步正常；而在 Wayland 下，未获焦点的应用能否读取剪贴板取决于合成器的 XWayland 桥接实现，因此把本机复制的内容广播出去可能并不可靠 —— 接收则不受影响。剪贴板历史目前只记录文本，也无法识别敏感内容标记。
